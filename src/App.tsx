...
function AttendanceTracker({ events, onRefresh, searchQuery }: { events: Event[], onRefresh: (silent?: boolean) => void, searchQuery: string }) {
  ...
  async function onScanSuccess(decodedText: string) {
    try {
      ...
      if (!participant) {
        setScanError("Not registered for this event!");
        setTimeout(() => setScanError(null), 3000);
        return;
      }

      if (participant.status === 'attended') {
        setScanSuccess(`${participant.name} is already present.`);
      ...
      if (success) {
        setScanSuccess(`Welcome, ${participant.name}!`);
        setTimeout(() => setScanSuccess(null), 3000);
      }
    } catch (err) {
      ...
      {isScanning && (
        <div className="fixed inset-0 bg-black z-[100] flex flex-col">
          <div className="p-6 flex justify-between items-center text-white">
            <h3 className="text-xl font-bold">Scan Participant QR</h3>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-4">
            <div id="reader" className="w-full max-w-md bg-white rounded-2xl overflow-hidden shadow-2xl"></div>
            ...
            <div className="mt-8 w-full max-w-md">
              {scanError && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="p-4 bg-red-500/20 border border-red-500 text-red-100 rounded-xl text-center font-medium"
                >
                  {scanError}
                </motion.div>
              )}
              {scanSuccess && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="p-4 bg-emerald-500/20 border border-emerald-500 text-emerald-100 rounded-xl text-center font-medium"
                >
                  {scanSuccess}
                </motion.div>
              )}
          ...
          <div className="p-8 text-center text-slate-400 text-sm">
            Point your camera at the participant's registration QR code.
          </div>
        </div>
      )}
